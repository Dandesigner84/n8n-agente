...
import { Server, Key, ArrowRight, ExternalLink, Workflow } from 'lucide-react';
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onLogin({ baseUrl: formattedUrl, apiKey });
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#111827] p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#1a202c] border border-gray-700 rounded-2xl shadow-2xl z-10 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            <div>
                ...
                <div className="relative group">
                    ...
                    <input 
                        ...
                        className="w-full bg-[#111827] border border-gray-600 rounded-lg py-2.5 pl-10 pr-4 text-white focus:ring-2 focus:ring-[#ff6d5a]/50 focus:border-[#ff6d5a] outline-none transition-all placeholder-gray-600"
                    />
                </div>
            </div>
            ...
            {error && (
                <div className="p-3 bg-red-900/20 border border-red-800 rounded-lg text-red-300 text-xs text-center">
                    {error}
                </div>
            )}
            ...
            <button 
                type="submit"
                className="w-full bg-[#ff6d5a] hover:bg-[#ff8f7e] text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-2 transition-all transform hover:scale-[1.02] shadow-lg shadow-[#ff6d5a]/20"
            >
                Conectar e Acessar
                <ArrowRight size={18} />
            </button>
      ...
      <div className="mt-8 text-gray-600 text-xs text-center max-w-xs">
        Ao conectar, seus dados são salvos apenas localmente no seu navegador para permitir a comunicação da IA.
      </div>
    </div>
  );
};
